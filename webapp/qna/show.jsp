...
<s:setDataSource url="jdbc:oracle:thin:@127.0.0.1:1521:orcl"
	driver="oracle.jdbc.driver.OracleDriver" user="root"
	password="TPqls5144" var="db" scope="page" />

<sql:query var="records" dataSource="${db}" />
					...
					<article class="article">
						...
						<div class="article-util">
							<ul class="article-util-list">
								<li><a class="link-modify-article" href="#">수정</a></li>
								<li>
							...
							<div class="qna-comment-slipp-articles">
								<article class="article">
									<div class="article-header">
										...
										<div class="article-header-text">
											<sql:forEach var="value1" items="${record.rows}">
												value1="${value.[writer]}"
												<p>value1</p>
											</sql:forEach>
											<div class="article-header-time">
												<sql:forEach var="value2" items="${record.rows}">
											value2="${value.[contents]}"
											<p>value2</p>
												</sql:forEach>
											</div>
										</div>
									...
									<div class="article-doc comment-doc">
										<sql:forEach var="value3" items="${record.rows}">
											value3="${value.[contents]}"
											<p>value3</p>
										</sql:forEach>
									</div>
									<div class="article-util">
										<ul class="article-util-list">
											<li><a class="link-modify-article"
												href="/api/qna/updateAnswer?answerId=5">수정</a></li>
											<li>
								...
								<article class="article">
									...
									<div class="article-doc comment-doc">
										<p>람다식에서 사용되는 변수라면 람다식 내부에서 정의된 로컬 변수이거나 람다식이 선언된 외부의 변수를
											참조하는 것일텐데, 전자라면 아무리 변경해도 문제될 이유가 없고, 후자는 변경 자체가 허용이 안될텐데. 이
											설명이 무슨 뜻인지 이해가 안 됨.</p>
									</div>
									<div class="article-util">
										<ul class="article-util-list">
											<li><a class="link-modify-article"
